...
SET @table_query = CONCAT(
  'SELECT ',
  '  CONCAT(\'{"name":"\', name, \'","request":{"url":"\', request_url, \'","method":"GET"},"response":[]}\') AS item ',
  'FROM (',
...
SET @json = CONCAT(
  ...
  '    ', (SELECT GROUP_CONCAT(item SEPARATOR ',\n')
            FROM (SELECT TABLE_NAME AS name,
                         ...
                         CONCAT('{"name":"', TABLE_NAME, '","request":{"url":"{{base_url}}/', REPLACE(TABLE_NAME, '_', '-'), '","method":"GET"},"response":[]}') AS item
                  FROM INFORMATION_SCHEMA.TABLES
